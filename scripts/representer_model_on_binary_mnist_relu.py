...
model = tf.keras.Sequential(
    [
        ...
        tf.keras.layers.Dense(4, kernel_regularizer="l2", activation="relu", use_bias=False),
        tf.keras.layers.Dense(4, kernel_regularizer="l2", activation="relu", use_bias=False),
...
model.load_weights("./output/binary_mnist_relu_checkpoint")

num_training_points = 13007
num_test_points = 2163

feature_model = tf.keras.Sequential(model.layers[0:3])
prediction_network = model.get_layer(index=3)
...
np.savez(
    "./output/representer_model_on_binary_mnist_relu.npz",
